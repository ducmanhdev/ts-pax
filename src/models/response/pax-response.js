...
const pax_batch_response_1 = __importDefault(require("./pax-batch-response"));
const pax_payment_response_1 = __importDefault(require("./pax-payment-response"));
class PaxResponse {
    ...
    static fromString(res) {
        ...
        if (fields.length >= 5) {
            ...
            if (fields.length === 14 &&
                ...
                result.paxPaymentResponse = pax_payment_response_1.default.fromList(fields);
                result.amount = parseInt(dataAmount[0]);
            }
            else if (fields.length === 12 &&
                command === PaxResponse.COMMAND_TYPE_BATCH) {
                result.paxBatchResponse = pax_batch_response_1.default.fromList(fields);
            }
            // COMMAND PAX BATCH
            return result;
        }
